...
這是一個過渡版本，會將已有的用戶詞典升級爲新的格式，而舊式的用戶詞典（`*.userdb.kct`）今後將不再支持。
爲保證完成過渡，從 0.9.26 版本以後鼠鬚管將啓用新的推送更新頻道，如果需要繼續接收推送，請勿跳過本次升級。

——真的想直接升級到後續版本…
——請在升級安裝前，手動執行一次用戶數據同步，並在升級安裝後再將數據同步到新生的用戶詞典。

----------

下載：[Bintray](http://dl.bintray.com/lotem/rime/Squirrel-0.9.26.zip) | [百度雲分享鏈接](http://yun.baidu.com/s/1mg2yDc8)
sha1: c0c704169cae07d96cf465e3b60eae18daaa5332
